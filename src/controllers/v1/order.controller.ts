...
import Order from '../../services/v1/order.service';
...
const getById: RequestHandler = AsyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req?.params;
  const result = await Order.getById(req?.user?.id, new Types.ObjectId(id));
  res.status(HttpCode.OK).json({ success: true, message: '', data: result });
});
...
const edit: RequestHandler = AsyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req?.params;
  const result = await Order.edit(req?.user?.id, new Types.ObjectId(id), req?.body);
...
const remove: RequestHandler = AsyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req?.params;
  const result = await Order.remove(req?.user?.id, new Types.ObjectId(id));
  res.status(HttpCode.OK).json({
    success: true,
